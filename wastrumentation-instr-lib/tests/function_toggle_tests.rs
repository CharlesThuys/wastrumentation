...
use wasabi_wasm::Instr::{Binary, Call, CallIndirect, Const, End, RefFunc};
...
use wasabi_wasm::{Element, LoadOp, Memarg, Memory, StoreOp};
...
fn instrument_test() -> Result<(), wastrumentation::error::InstrumentationError> {
    ...
    let (mut module, _offsets, _issue) =
        M2::from_bytes(&wat_input_program).map_err(InstrumentationError::ParseModuleError)?;
    ...
    let table_funs_refs: Vec<Vec<Instr>> = original_idxs
        .into_iter()
        .flat_map(|idx| {
            let instr_idx = *additional_functions.get(&idx).unwrap_or(&idx);
            [vec![RefFunc(idx), End], vec![RefFunc(instr_idx), End]]
        })
        .collect();
    ...
    let memory = Memory::new(Limits {
        initial_size: original_function_count as u32,
        max_size: None,
    });
            ...
            for instr in mut_code.body.iter() {
                match instr {
                    Call(index) => {
                        ...
                        // LOAD FLAG VALUE
                        new_body.push(Instr::Load(
                            LoadOp::I32Load8U,
                            Memarg::default_with_index(LoadOp::I32Load8U, memory_idx),
                        ));
                        ...
                        new_body.push(Instr::Store(
                            StoreOp::I32Store8,
                            Memarg::default_with_index(StoreOp::I32Store8, memory_idx),
                        ));
...
fn block_test() -> Result<(), wastrumentation::error::InstrumentationError> {
    ...
    assert_eq!(wasm_call!(store, add, (9, 491)), 500);

...
fn main() -> io::Result<()> {
    ...
    let mut size_wtr =
        csv::Writer::from_path(RESULT_DIR.to_owned() + BENCH_NAME  + &time.to_rfc3339() + "/baseline_sizes.csv")?;
    let mut wtr =
        csv::Writer::from_path(RESULT_DIR.to_owned() + BENCH_NAME + &time.to_rfc3339() + "/baseline_result.csv")?;
    ...
    for program_path in programs {
        ...
        for analysis_path in &analyses {
            ...
            // Run wasmtime
            for run in 0..RUNS {
                let result = run_bench("wasmtime", program_name, &instrumented, analysis_name, "baseline", run as u32);
                wtr.serialize(result)?;
                wtr.flush()?;
            }
            // Run wizard
            for run in 0..RUNS {
                let result = run_bench("wizard", program_name, &instrumented, analysis_name, "baseline", run as u32);
                wtr.serialize(result)?;
                wtr.flush()?;
            }
